...
namespace InGame {
	...
	WindChecker::WindChecker(const InitiateData& initData)
	{
		...
		glm::vec3 position( 0.8f, WindYPost, ZOrder::z_FlatChecker);
		...
		glm::vec3 leftPosition(0.711f, WindYPost, ZOrder::z_FlatCheckerImg);
		...
		glm::vec3 rightPosition(0.889f, WindYPost, ZOrder::z_FlatCheckerImg);
		...
		glm::vec3 leftMaskPosition(m_LeftMaskBasis, WindYPost, ZOrder::z_FlatFont);
		glm::vec3 rightMaskPosition(m_RightMaskBasis, WindYPost, ZOrder::z_FlatFont);
	...
	void WindChecker::LeftWindMaskUpdate()
	{
		...
		{
			m_LeftMaskTranslate = glm::translate(glm::mat4(1.0f), glm::vec3(m_LeftMaskBasis + 0.088f * m_CurrentWind / m_WindMax, WindYPost, ZOrder::z_FlatFont)) *
	...
	void WindChecker::RightWindMaskUpdate()
	{
		if (m_CurrentWind > 0.0f)
		{
			m_RightMaskTranslate = glm::translate(glm::mat4(1.0f), glm::vec3(m_RightMaskBasis + 0.088f * m_CurrentWind / m_WindMax, WindYPost, ZOrder::z_FlatFont)) *
				glm::scale(glm::mat4(1.0f), glm::vec3(m_maskWidthUnit * (1.0f - m_CurrentWind / m_WindMax), m_maskHeightUnit, 1.0f));
